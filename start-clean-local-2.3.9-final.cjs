// Clean local static server for Fight World FINAL VERSION FIX 2.3.9
...
if (!fs.existsSync(path.join(ROOT, "index.html")) || !fs.existsSync(path.join(ROOT, "reset-cache.html"))) {
  throw new Error("Run this from the FightSimulator repository root after node apply-2.3.9-final.cjs");
}
...
server.listen(PORT, HOST, () => {
  const url = `http://${HOST}:${PORT}/reset-cache.html?target=2.3.9&t=${Date.now()}`;
