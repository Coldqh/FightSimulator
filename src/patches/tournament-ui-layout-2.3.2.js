(function () {
  ...
  window.FS = window.FS || {};
  var FS = window.FS;
  var PATCH_VERSION = "update-button-fix-2.3.2";
  var MONTH_WEEKS = 4;
  ...
  function snapshotSchedules(week) {
    ...
    competitions().forEach(function (comp) {
      if (ruleFor(comp) && !scheduledAt(comp, week)) {
        comp.schedule = "__disabled_by_calendar_2_3_2__";
      }
    });
  ...
  function patchAmateurCalendar() {
    ...
    if (!Amateur || Amateur.__calendar232) {
      return;
    }
    Amateur.__calendar232 = true;
  ...
  function patchWorldCalendar() {
    var World = FS.World;
    if (!World || World.__calendar232) {
      return;
    }
    World.__calendar232 = true;
  ...
  function injectStyles() {
    var style;
    if (document.getElementById("fight-world-hotfix-2-3-2")) {
      return;
    }

    style = document.createElement("style");
    style.id = "fight-world-hotfix-2-3-2";
    style.textContent = [
  ...
  function patchRender() {
    ...
    if (!Render || Render.__hotfix232 || typeof Render.dashboard !== "function") {
      return;
    }
    Render.__hotfix232 = true;
