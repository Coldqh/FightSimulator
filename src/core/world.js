(function () {
  ...
  function createNews(state, tone, text, meta) {
    U.pushLimited(state.world.news, { id: U.uid("news"), week: state.week, tone: tone || "world", text: text, meta: meta || {} }, 80);
  }

  function findCloseOpponent(state, sourceFighter) {
    var pool = state.roster.filter(function (fighter) {
      return fighter.id !== sourceFighter.id && !fighter.isPlayer && fighter.countryId === sourceFighter.countryId && fighter.trackId === sourceFighter.trackId && fighter.weightClassId === sourceFighter.weightClassId;
    });
  ...
  function simulateNpcTraining(state) {
    var count = Math.min(24, Math.max(8, Math.floor(state.roster.length / 42)));
    ...
    for (i = 0; i < count; i += 1) {
      ...
      key = U.pick(keys); cap = U.findTrack(fighter.trackId).maxStat;
  ...
  function simulateNpcFights(state) {
    var fighters = state.roster.filter(function (fighter) { return !fighter.isPlayer && state.week - (fighter.lastFightWeek || 0) >= 2; });
    var used = {};
    var report = [];
    ...
    for (i = 0; i < count; i += 1) {
      ...
      if (report.length <= 5) { createNews(state, "fight", result.text, { type: "npc_fight" }); }
    }
    return report;
  }
  ...
  function tryMoveFighter(state, fighter, targetTrackId, reason) {
    if (!canMoveToTrack(fighter, targetTrackId)) { return false; }
    if (fighter.trackId === "pro" && targetTrackId === "street") { fighter.proClosed = true; }
    if (targetTrackId === "pro") { fighter.hasGonePro = true; }
    fighter.trackId = targetTrackId;
    fighter.lastMoveWeek = state.week;
    State.updateDerivedFighterFields(fighter);
    fighter.careerLog.unshift({ week: state.week, text: reason });
    U.pushLimited(state.world.transitionLog, { id: U.uid("move"), week: state.week, fighterId: fighter.id, text: fighter.name + ": " + reason }, 60);
    createNews(state, "move", fighter.name + ": " + reason, { type: "track_move" });
    return true;
  }

  function simulateTransitions(state) {
    var candidates = state.roster.filter(function (fighter) { return !fighter.isPlayer && state.week - (fighter.lastMoveWeek || 1) >= 5; });
    var attempts = Math.min(8, candidates.length);
    var i, fighter, rating;
  ...
  function buildNationalTeams(state) {
    ...
    for (i = 0; i < Data.countries.length; i += 1) {
      ...
      pool = State.ranking(state, country.id, "amateur", "").filter(function (fighter) { return U.statAverage(fighter.stats) >= 40; });
      teams[country.id] = { main: pool.slice(0, 4).map(function (fighter) { return fighter.id; }), reserve: pool.slice(4, 8).map(function (fighter) { return fighter.id; }) };
    }
  ...
  function buildOfferOpponent(state, index) {
    ...
    var candidates = state.roster.filter(function (fighter) {
      return !fighter.isPlayer && fighter.countryId === p.countryId && fighter.trackId === p.trackId && fighter.weightClassId === p.weightClassId;
    });
    candidates.sort(function (left, right) { return Math.abs(U.scoreFighter(left) - targetScore) - Math.abs(U.scoreFighter(right) - targetScore); });
    if (!candidates[index]) {
      candidates[index] = State.createFighter(p.countryId, p.trackId, 9000 + state.week * 10 + index, U.statAverage(p.stats) + difficulty.offset, { weightClassId: p.weightClassId });
  ...
  function refreshOffers(state) {
    var p = State.player(state);
    var labelsByTrack = { amateur: ["Любительский бой", "Бой городского уровня", "Матч отбора"], street: ["Дворовый бой", "Районный вызов", "Бой на местной площадке"], pro: ["Профессиональный андеркард", "Контрактный бой", "Главный бой вечера"] };
    var track = U.findTrack(p.trackId);
    var i, opponent, difficulty;
    state.offers = [];
    for (i = 0; i < 3; i += 1) {
      difficulty = Data.offerDifficulties[i] || Data.offerDifficulties[1];
      opponent = buildOfferOpponent(state, i);
      state.offers.push({ id: U.uid("offer"), label: labelsByTrack[p.trackId][i], difficultyId: difficulty.id, opponentId: opponent.id, rounds: track.rounds, purse: Math.max(0, Math.round((track.basePurse + i * Math.round(track.basePurse * 0.35)) * difficulty.purseMul)), risk: Math.max(1, U.statAverage(opponent.stats) - U.statAverage(p.stats) + 50) });
    }
  }
  ...
  function advanceWeek(state, action) {
    ...
    refreshOffers(state);
    U.pushLimited(state.world.weekReports, { id: U.uid("week"), week: state.week, action: action || "week", fights: npcReport.slice(0, 8) }, 30);
  }
  ...
  function bootstrapWorld(state) {
    ...
    buildNationalTeams(state);
    refreshOffers(state);
    if (!state.world.news.length) { createNews(state, "world", "Мир запущен: рейтинги, сборные, весовые категории и расписание боёв сформированы.", { type: "bootstrap" }); }
  }

  window.FS.World = { createNews: createNews, refreshOffers: refreshOffers, advanceWeek: advanceWeek, bootstrapWorld: bootstrapWorld, simulateNpcTraining: simulateNpcTraining, simulateNpcFights: simulateNpcFights, simulateTransitions: simulateTransitions, buildNationalTeams: buildNationalTeams };
}());
